    ...
    class Program
    {
        ...
        static void Register()
        {
            ...
            if (t.Result.StatusCode == StatusCode.REGISTERED)
            {
                ...
                custCode = t.Result.CustCode;
                depId = t.Result.DepId;
                depName = t.Result.DepName;
            }
        ...
        {
            // sample notification
            NotificationContent nc = new NotificationContent { SvyId = 1, SvyText = "Ist N.H. ein Nazi?", EndDate = DateTime.Parse("2019-08-24"), CatName = "Testfrage", TypeId = 1, PossibleTextAnswers = null };
            ...
            WriteLine(nc.SvyText);
            WriteLine("Possible answers:");
            predefinedAnswers.Where(a => a.TypeId == nc.TypeId).ToList().ForEach(a =>
            {
                WriteLine("  " + a.AnsDesc);
            });
            ...
            {
                ans = ReadLine();
                ansId = predefinedAnswers.Where(a => a.AnsDesc.ToLower() == ans.ToLower()).Select(a => a.AnsId as int?).FirstOrDefault();
            } while (ansId == null);

            hub.Invoke("AnswerSurvey", new Vote { }).Wait();
